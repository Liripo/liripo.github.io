---
title: "用 Gunicorn 部署 Dash 生产环境"
subtitle: "Dash 应用的多进程启动、配置文件和常用信号"
description: "整理 Gunicorn 部署 Dash 应用时需要暴露的 server 对象、配置文件示例以及平滑重启信号。"
author: "Liripo"
date: "2024-06-15"
date-modified: "2026-06-01"
categories: [dash, web, python, gunicorn]
image: "dash.jpg"
---

## 使用 Gunicorn 部署 Dash

Gunicorn 的好处是配置简单，支持多进程，并且可以通过信号完成平滑重启。


1. 首先，Dash 的代码中需要暴露 Flask `server` 对象。例如 `app.py` 中必须包含下面代码：

```py
app = Dash(__name__)
server = app.server # Gunicorn 会加载这个对象
```
