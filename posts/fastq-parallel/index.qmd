...
cutadapt[^cutadapt] 已经把这些都实现好了。它的并行框架设计得很干净，核心只需理解三个概念：**Modifier**、**Step** 和 **Pipeline**。
...
- **ReaderProcess**：独立进程，用 dnaio[^dnaio] 的 `read_chunks()` 按块读取 FASTQ，通过 `Connection` pipe 发送给空闲 worker
...
[^cutadapt]: [cutadapt](https://github.com/marcelm/cutadapt) — adapter 修剪与并行 FASTQ 处理框架
[^PISA]: [PISA](https://github.com/shienex/PISA) — k-mer 索引 barcode 校正
[^dnaio]: [dnaio](https://github.com/marcelm/dnaio) — FASTQ/FASTA 读写库
