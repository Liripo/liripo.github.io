...
其中 $d_i = rank(x_i) - rank(y_i)$，即基因 i 在两条表达向量中的秩差。$\sum d_i^2$ 是秩差的平方和，也就是秩向量之间**欧氏距离的平方**。

反过来看：两个细胞的秩向量越接近，欧氏距离越小，Spearman 相关系数就越接近 1。所以找相关最高的参考细胞 = 在秩空间里找欧氏距离最近的邻居。

这样一来，分类问题就变成了"在秩空间里搜最近邻"——这是个已经研究透的问题，有大量高效的 ANN 算法可以用。

#### singlepp 的具体做法

训练时，`train_single` 把所有参考细胞的秩值建成一个统一的 KNN 索引（用的是 DE 基因子集对应的秩值）。

分类时，`classify_single` 对每个测试细胞：

1. 提取 DE 基因的表达值，转成秩
2. 在整个索引里搜 K 个最近邻的参考细胞（不管什么标签，全局搜最近的 K 个）
3. 按标签分组：每个标签拿出属于它的那部分邻居，KNN 返回的欧氏距离平方后代入 $\rho = 1 - 6\sum d_i^2 / (n(n^2 - 1))$ 得到 Spearman 相关
4. 每个标签内的 K 个相关值取分位数（默认 0.8），得到该标签的最终得分
5. 得分最高的标签就是初版注释
