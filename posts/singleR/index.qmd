...
2 个类型时 $n = 500$，16 个类型时 $n \approx 125$。类型少，需要更多基因来区分；类型多，不同基因的组合本身就够了，不需要那么多。
...
`cor.stable` 是 `cor()` 的稳健封装。输出一个 $M \times K$ 矩阵，一行一个测试细胞，一列一个参考样本，里面是 Spearman 秩相关系数。
...
为什么不用均值？参考数据里一个类型可能有多个样本，有些高分有些低分（批次效应之类的）。取 0.8 分位，相当于定位在"最相似的那 20% 参考样本"的水平——对离群值不敏感，但比中位数更靠近高相关的那端。跟 [CellTypist](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7613558/) 里的"投票"思路有点像。
...
T-cell 和 NK-cell 都在 0.05 差额内，B-cell 出局。第二轮只在 T 和 NK 之间找差异基因重新跑。这种"聚焦再聚焦"的打法，在 CD4+/CD8+ T 这种姐妹亚群区分上效果很好。
...
**classic**（默认）：跟 legacy 一样，中位表达差排序。`de.n` 默认 $500 \times (2/3)^{\log_2(N)}$。适合 bulk 参考。
...
Fine-tuning 逻辑跟 legacy 一样，全用 C++ 实现。`tune.thresh = 0.05` 控制候选筛选。
